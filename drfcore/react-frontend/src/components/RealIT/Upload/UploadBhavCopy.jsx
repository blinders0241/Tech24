...
function UploadBhavCopy() {
  ...
  const handleSubmit = async (e) => {
    ...
    setUploading(true);
    try {
      const response = await axios.post(allNotesapi + "uploadData/", formData);
  ...
  const handleClearDB = async (e) => {
    if (window.confirm("Are you sure you want to execute this action?"))
      try {
        const response = await axios.get(allNotesapi + "delete/");
        setStatus(response);
        // handle success
  ...
  return (
    <>
      <Container>
        <Row>
          <Col md={6}>
            <h1>Upload Derivatives BhavCopy </h1>
